...
def load_point_cloud(las_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
    """지정한 디렉터리에서 LAS/LAZ 파일들을 로드하여
    coords(xyz), colors(rgb)를 numpy 배열로 반환하는 헬퍼.

    기존 파이프라인의 "coords, colors = load_point_cloud(las_dir)" 패턴을
    그대로 사용할 수 있도록 `load_las_directory()`에 대한 래퍼를 제공한다.
    """
    # 내부는 GPU/CPU 여부에 상관없이 numpy로 반환
    xyz_gpu, rgb_gpu = load_las_directory(las_dir, use_gpu=True)

    # CuPy 배열이면 .get(), 아니면 그대로 numpy로 캐스팅
    if hasattr(xyz_gpu, "get"):
        coords = xyz_gpu.get()
    else:
        coords = np.asarray(xyz_gpu)

    if hasattr(rgb_gpu, "get"):
        colors = rgb_gpu.get().astype(np.uint8)
    else:
        colors = np.asarray(rgb_gpu, dtype=np.uint8)
