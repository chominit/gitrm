...
def process_rays_batch_gpu(rays: 'cp.ndarray', origin: 'cp.ndarray',
    ...
    MAX_POINTS_PER_BATCH = 50_000  # 기본값
    ...
    try:
        ...
        if available_for_us > 0:
            # float32 기준 per (ray, point) 조합 대략 메모리(보수적으로 32 bytes 가정)
            BYTES_PER_PAIR = 32
...
def process_image_scanline(img_name: str, cam_info: dict, coords: np.ndarray,
    ...
    for row in tqdm(rows_with_pixels, desc=f"   Processing {img_name}", leave=False):
        ...
        pixels_hom = np.stack([x_indices, y_indices, np.ones(num_rays)], axis=1).astype(np.float32)
        ...
        band_coords_gpu = cp.asarray(band_coords, dtype=cp.float32)
