    ...
    try:
        ...
        if camera_model is not None:
            ...
            if camera_model.find('k1') is not None:
                ...
                distortion = cp.array([k1, k2, p1, p2, k3], dtype=cp.float64)
        else:
            # 기본값 사용
            focal_length = 35.0
            sensor_width = 36.0
        ...
        if calibrated_cameras is not None:
            for camera in calibrated_cameras.findall('camera'):
                ...
                if position_elem is not None:
                    ...
                    position = cp.array([x, y, z], dtype=cp.float64)
                else:
                    position = cp.zeros(3, dtype=cp.float64)
                ...
                if rotation_elem is not None:
                    ...
                    rotation = cp.array([omega, phi, kappa], dtype=cp.float64)
                else:
                    rotation = cp.zeros(3, dtype=cp.float64)
...
def load_camera_parameters(
    ...
    for img_name, params in camera_dict.items():
        cam = CameraParameters(
            image_name=img_name,
            position=cp.array(params['position'], dtype=cp.float64),
            rotation=cp.array(params['rotation'], dtype=cp.float64),
            focal_length=params['focal_length'],
            sensor_width=params['sensor_width'],
            sensor_height=params['sensor_height'],
            principal_point=tuple(params['principal_point']),
            distortion=cp.array(params['distortion'], dtype=cp.float64) if params['distortion'] else None
        )
...
def load_camera_db(site_name: str) -> Dict:
    ...
    # 통합 B/C 파일 우선 사용
    if "Site_B" in site_name or "Site_C" in site_name:
        eop_path = current_dir / "Site_B_C_Images_EOP.txt"
        if eop_path.exists():
            logger.info(f"EOP 파싱: {eop_path.name} (B/C 통합)")
            cams = _parse_simple_eop(eop_path)
            cams['images'] = filter_cameras_by_site(cams['images'], site_name)
            return cams
