...
def camera_to_image(
    ...
    # 정규화된 이미지 좌표
    normalized = cp.zeros((len(camera_points), 2), dtype=cp.float64)
...
def image_to_ray(
    ...
    """
    # 정규화된 이미지 좌표
    normalized = cp.zeros((len(image_points), 3), dtype=cp.float64)
...
def compute_z_buffer(
    ...
    # Z-버퍼 초기화 (무한대)
    z_buffer = cp.full(image_size, cp.inf, dtype=cp.float64)
