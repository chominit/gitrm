...
def process_rays_chunk_gpu(rays_cpu: np.ndarray,
    ...
    rays_gpu = cp.asarray(rays_cpu, dtype=cp.float64)
    origin_gpu = cp.asarray(origin_cpu, dtype=cp.float64)
    ...
    for start in range(0, M_total, point_batch_size):
        ...
        pts_batch = cp.asarray(coords_batch_cpu, dtype=cp.float64)  # (M_batch, 3)
...
def process_image_chunked(img_name: str,
    ...
    # Row chunk 루프
    for chunk_idx, row_start in enumerate(range(0, IMG_HEIGHT, chunk_size), start=1):
        ...
        # 진행 상황 로그 (너무 자주 찍지 않음)
        if chunk_idx % 20 == 0 or row_end == IMG_HEIGHT:
            print(
                f"      row chunk {chunk_idx}: rays {num_rays:,}, band_pts {len(band_coords):,}",
                flush=True,
            )
