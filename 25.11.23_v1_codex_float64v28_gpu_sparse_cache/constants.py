...
# ==============================
# CPU 스레딩 설정 (기본: 전체 코어 사용, 환경변수로 조정 가능)
# ==============================
_cpu_count = os.cpu_count() or 1
_cpu_fraction = float(os.environ.get("CPU_THREAD_FRACTION", "0.95"))  # 기본 95%, 예: 0.9 = 90%
_cpu_threads = max(1, int(_cpu_count * _cpu_fraction))
...
# ==============================
PARALLEL_CONFIG = {
    "num_workers": 28,         # 기본 워커 수 (환경변수 IMAGE_WORKERS로 덮어씀)
...
# VRAM 여유 확보: 배치 크기 더 축소 (메모리 스필 방지)
MAX_POINTS_PER_BATCH = 2_000_000  # 배치당 최대 포인트 수

# 메모리 제한 설정 (사용자 환경 기준)
# CPU RAM 최대 사용량 (bytes) - 300GB
MAX_CPU_MEMORY_BYTES: int = 300 * 1024**3

# GPU VRAM 최대 사용 비율 (예: 0.90 = 90%)
MAX_GPU_MEMORY_USAGE_RATIO: float = 0.90
