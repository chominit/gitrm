    ...
    try:
        ...
        with laspy.open(filepath, mode='r') as las_file:
            ...
            if use_gpu and GPU_AVAILABLE:
                xyz = cp.asarray(xyz_np, dtype=cp.float64)
...
def load_point_cloud(las_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    우선순위
    1) NPY 캐시 (coords_float64.npy / coords_float32.npy, colors_uint8.npy)
    2) 없으면 LAS 디렉터리 스캔(load_las_directory)

    반환은 항상 numpy float64(coords), uint8(colors).
    """
    coords_f64 = las_dir / "coords_float64.npy"
    coords_f32 = las_dir / "coords_float32.npy"
    colors_npy = las_dir / "colors_uint8.npy"

    if coords_f64.exists() and colors_npy.exists():
        coords = np.load(coords_f64, mmap_mode='r').astype(np.float64, copy=False)
        colors = np.load(colors_npy).astype(np.uint8, copy=False)
        logger.info(f"NPY 로드 성공: {coords_f64.name}, {colors_npy.name}")
        return coords, colors

    if coords_f32.exists() and colors_npy.exists():
        coords = np.load(coords_f32, mmap_mode='r').astype(np.float64, copy=False)
        colors = np.load(colors_npy).astype(np.uint8, copy=False)
        logger.info(f"NPY(float32) 로드 후 float64 캐스팅: {coords_f32.name}")
        return coords, colors

    logger.warning("NPY 캐시를 찾지 못했습니다. LAS 디렉터리를 스캔합니다.")
    xyz_gpu, rgb_gpu = load_las_directory(las_dir, use_gpu=True)

    if xyz_gpu.size == 0:
        raise FileNotFoundError(
            f"포인트 클라우드 없음: {las_dir} (NPY와 LAS 모두 찾지 못했습니다)"
        )

    coords = xyz_gpu.get() if hasattr(xyz_gpu, "get") else np.asarray(xyz_gpu)
    colors = rgb_gpu.get() if hasattr(rgb_gpu, "get") else np.asarray(rgb_gpu)
    return coords.astype(np.float64, copy=False), colors.astype(np.uint8, copy=False)


def save_las(coords: np.ndarray,
             colors: np.ndarray,
             filepath: Path) -> None:
    """기존 write_las_file()를 감싸는 단순 래퍼.

    coords, colors가 numpy 배열이든 CuPy 배열이든 상관없이
    write_las_file()에서 내부적으로 처리할 수 있도록 전달한다.
    """
    write_las_file(filepath, coords, colors)
