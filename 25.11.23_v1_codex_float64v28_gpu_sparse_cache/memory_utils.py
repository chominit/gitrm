...
# 시스템 RAM 상한 (256GB에 맞춤)
CPU_HARD_LIMIT_GB: float = 256.0

# GPU VRAM 사용 비율 (예: 0.95 = 총 VRAM의 95%까지만 사용)
GPU_MEM_USAGE_RATIO: float = 0.95

# Ray–Point 쌍당 예상 메모리 사용량 (bytes, float64 기준 보수적 추정)
# 행렬 계산/캐싱을 감안한 64B 근사값으로, VRAM 사용을 안전하게 제한한다.
BYTES_PER_PAIR_ESTIMATE: int = 64

# 기본 포인트 배치 크기 (4090 24GB 기준 여유 있게 확장)
DEFAULT_BATCH_SIZE: int = 120_000
