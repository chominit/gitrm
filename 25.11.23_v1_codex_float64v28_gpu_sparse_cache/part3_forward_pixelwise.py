...
def forward_vote_pixelwise_gpu(
    ...
    for img_idx, img_name in enumerate(tqdm(image_names, desc="   [GPU] Footprint-based", unit="img")):
        ...
        # 5. Ray 생성 (GPU)
        C_cam_gpu = cp.asarray(C_cam_np, dtype=cp.float64)
        R_c2w_gpu = cp.asarray(R_c2w_np, dtype=cp.float64)

        ones = cp.ones(num_pixels, dtype=cp.float64)
        ...
        # 6. Subset을 GPU로 전송
        subset_coords_gpu = cp.asarray(subset_coords, dtype=cp.float64)
...
def process_rays_brute_force_gpu(
    ...
    # float64 기준 필요 메모리: N × M_batch × 8 × (1 + 3 + 3) = N × M_batch × 56 bytes
    BYTES_PER_ELEMENT = 56  # proj_dist + proj_points + residuals
